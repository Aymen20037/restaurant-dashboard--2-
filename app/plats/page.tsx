...
// Correction : ingredients est une string
interface Dish {
  id: string;
  name: string;
  price: number;
  ...
  ingredients: string | null; // <-- correction
  isAvailable: boolean;
}
  ...
  async function handleAddDish(
    ...
    try {
      ...
      const payload = {
        ...
        preparationTime: newDish.preparationTime
          ? Number(newDish.preparationTime)
          : null,
        // on envoie un tableau d'ingrédients au backend
    ...
    try {
      ...
      setEditDish({
        ...
        preparationTime: data.dish.preparationTime
          ...
          : "",
        // convertir en string pour le textarea
  ...
  async function handleSubmitEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = {
        ...
        preparationTime: editDish.preparationTime
          ? Number(editDish.preparationTime)
          : null,
        ingredients: editDish.ingredients
          ? editDish.ingredients
              .split(",")
              .map((i) => i.trim())
              .filter(Boolean)
          : [],
              ...
              <DialogContent className="max-w-2xl">
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    handleAddDish(e, () => {
                      // fermeture du dialog via API Shadcn
                      const dialog = document.querySelector("button[data-state='open']");
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(val) =>
                          setNewDish((prev) => ({ ...prev, categoryId: val }))
                        }
                      >
                        <SelectTrigger>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div>
              ...
              <p>
                ...
                {selectedDish?.isAvailable ? "Oui" : "Non"}
              </p>
            </div>
            <Button
              className="mt-4"
